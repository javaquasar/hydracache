...
function OperationalPanels() {
  return (
    <>
      ...
      <Panel id="operations" title="Operations" description="Current process generation" aside={<span class="readonly-badge">read only</span>}><p data-testid="operations-generation" class="visually-associated">Current process generation</p><DataTable label="Operations" headings={["ID", "Type", "Scope", "State", "Progress", "Requested", "Accepted", "Started", "Terminal", "Source / completeness", "Reason / remediation"]} bodyTestId="operations-table" /></Panel>
      <Panel id="audit" title="Audit metadata" description="Management operations in this process only" aside={<TrustNote>redacted · bounded</TrustNote>}><p data-testid="audit-coverage" class="visually-associated">Management operations in this process only</p><DataTable label="Audit metadata" headings={["Event", "Sequence", "Operation", "Category", "Action", "Outcome", "Occurred", "Source"]} bodyTestId="audit-table" /></Panel>
    </>
  );
}
...
function InventoryPanels() {
  return (
    <>
      ...
      <Panel id="namespaces" title="Authorized namespaces and caches" description="Totals are computed only after caller-scope authorization" aside={<TrustNote>no keys, values or policy</TrustNote>}><DataTable label="Authorized namespaces" headings={["Namespace", "Caches", "Entries", "Logical", "Retained", "Entry quota", "Byte quota", "Rate limit", "Fair share", "Rejected", "Subscriptions", "Near-cache repairs", "Persistence", "Quality"]} bodyTestId="namespace-table" /><DataTable label="Authorized caches" headings={["Namespace", "Cache", "Entries", "Logical", "Retained", "Hit / miss / load", "TTL backlog", "Tag/index", "Conditional", "Idempotency", "Audit", "Backup age", "Load breaker", "Quality"]} bodyTestId="cache-table" /></Panel>
    </>
  );
}
