...
export const membersEnvelope = {
  ...
  data: {
    items: dashboardEnvelope.data.members.map((item, index) => ({
      ...
      product_version: index === 0 ? "0.72.0" : null,
      protocol_compatible: index === 0 ? true : null,
      draining: index === 0 ? false : null,
      open_fds: index === 0 ? 27 : null,
      thread_count: index === 0 ? 8 : null,
      task_count: index === 0 ? 14 : null,
      client_count: index === 0 ? 4 : null,
      partition_count: index === 0 ? 32 : null,
      config_digest: index === 0 ? "sha256-v1:abc123" : null,
      authority_epoch: 42,
      observation_seq: 9,
      formation: formation(item.node, index === 2 ? "blocked" : "serving", index === 2 ? "learner-behind" : null),
      timeline: [{ observation_seq: 9, authority_epoch: 42, serving: index === 2 ? "blocked" : "serving", blocker: index === 2 ? "learner-behind" : null }],
    })),
    next_cursor: null,
    truncated: false,
  },
...
export const clientsEnvelope = {
  ...
  data: {
    ...
    protocols: [
      ...
      { protocol: "hc2", version: "hc-2-alpha", active_connections: 1, accepted_total: 4, closed_total: 3, rejected_total: 2, pending_invocations: 0, active_subscriptions: 1, active_sessions: 2, buffered_bytes: 2048 },
