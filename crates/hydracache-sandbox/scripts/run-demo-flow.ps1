...
Write-Host "`n4. Raw local cache put/get"
...
Write-Host "`n9. Single-flight scenario"
...
Write-Host "`n10. Invalidation/load race scenario"
...
Write-Host "`n11. Update backing store without invalidating cache"
Invoke-SandboxJson POST "/demo/users/42" @{ name = "Grace" } | ConvertTo-Json -Depth 8

Write-Host "`n12. Load still returns cached value"
Invoke-SandboxJson POST "/demo/load/42" | ConvertTo-Json -Depth 8

Write-Host "`n13. Invalidate user tag"
Invoke-SandboxJson POST "/demo/invalidate/user/42" | ConvertTo-Json -Depth 8

Write-Host "`n14. Reload returns updated backing store value"
Invoke-SandboxJson POST "/demo/load/42" | ConvertTo-Json -Depth 8

Write-Host "`n15. Application report"
Invoke-SandboxJson GET "/demo/report" | ConvertTo-Json -Depth 8

Write-Host "`n16. Actuator diagnostics"
