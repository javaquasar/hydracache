...
const W10_LOCAL_QUALIFICATION_AMENDMENT: &str =
...
pub fn check_at_root(
    ...
    let w10_local_qualification_amendment: TomlValue = toml::from_str(&fs::read_to_string(
        root.join(W10_LOCAL_QUALIFICATION_AMENDMENT),
    )?)?;
    ...
    problems.extend(check_w4_zero_copy_encode_contract(
    ...
    problems.extend(check_w10_local_qualification_amendment(
        &w10_local_qualification_amendment,
        release,
    ));
    problems.extend(check_w10_static_qualification_amendment(
    ...
    problems.extend(check_w10_focused_host_tooling_evidence(
...
pub fn check_w4_zero_copy_encode_contract(value: &TomlValue, release: &str) -> Vec<String> {
    let mut problems = Vec::new();
    if integer(value, "schema_version") != Some(1)
        || text(value, "release") != Some(release)
        || text(value, "contract_id") != Some("w4-zero-copy-encode-073-v1")
        || text(value, "state") != Some("preregistered-before-product-mutation")
        || text(value, "parent_contract") != Some(W4_RESP_TRANSLATION_PROFILE_CONTRACT)
        || text(value, "baseline_evidence") != Some(W4_RESP_TRANSLATION_PROFILE_EVIDENCE)
        || text(value, "source_parent") != Some("0a091e58b1749006ecd4d6c4106dff5d27fd240a")
        || text(value, "bytes_version") != Some("1.12.1")
    {
        problems.push("W4 zero-copy encode contract identity changed".to_owned());
    }
    for field in [
        "independent_processes_required",
        "prebuilt_release_binary_required",
        ...
        "all_invariants_required",
        "encode_wire_bytes_identical_required",
        "roundtrip_wire_bytes_identical_required",
        "binary_key_roundtrip_required",
        "resp2_resp3_semantic_equality_required",
        "duplicate_del_semantics_required",
        "product_mutation_allowed",
    ] {
        if boolean(value, field) != Some(true) {
            problems.push(format!("W4 zero-copy encode contract {field} must be true"));
        }
    }
    for field in [
        "production_counter_addition",
        ...
        if boolean(value, field) != Some(false) {
            problems.push(format!(
                "W4 zero-copy encode contract {field} must be false"
            ));
        }
    }
    if integer(value, "scenario_count") != Some(92)
        || integer(value, "repeats_per_scenario") != Some(5)
        || integer(value, "candidate_attempts") != Some(460)
        || string_array(value.get("large_response_cells"))
            != [
                "resp2/bulk/1048576",
                "resp2/array-16/1048576",
                "resp3/bulk/1048576",
                "resp3/array-16/1048576",
            ]
        || string_array(value.get("candidate_edits"))
            != [
                "encode_resp2_value: Ok(output.to_vec()) -> Ok(Vec::from(output))",
                "encode_resp3_value: Ok(output.to_vec()) -> Ok(Vec::from(output))",
            ]
    {
        problems.push("W4 zero-copy encode contract scope or matrix changed".to_owned());
    }
    if float(value, "minimum_large_response_gross_reduction_fraction") != Some(0.40)
        || float(value, "maximum_non_encode_gross_regression_fraction") != Some(0.05)
        || float(value, "maximum_small_encode_gross_regression_fraction") != Some(0.05)
        || integer(
            value,
            "baseline_resp2_bulk_1m_median_gross_bytes_per_operation",
        ) != Some(2_097_176)
        || integer(
            value,
            "baseline_resp2_array16_1m_median_gross_bytes_per_operation",
        ) != Some(2_098_122)
        || integer(
            value,
            "baseline_resp3_bulk_1m_median_gross_bytes_per_operation",
        ) != Some(2_097_176)
        || integer(
            value,
            "baseline_resp3_array16_1m_median_gross_bytes_per_operation",
        ) != Some(2_099_146)
    {
        problems.push("W4 zero-copy encode contract thresholds changed".to_owned());
    }
    for field in [
        "candidate_scope",
        "bytes_conversion_basis",
        "falsifier",
        "interpretation_limit",
        "decision_rule",
        "next_decision",
...
pub fn check_w10_local_qualification_amendment(value: &TomlValue, release: &str) -> Vec<String> {
    let mut problems = Vec::new();
    if integer(value, "schema_version") != Some(1)
        || text(value, "release") != Some(release)
        || text(value, "amendment_id") != Some("w10-local-qualification-amendment-6c107abf-v1")
        || text(value, "state")
            != Some("preregistered-after-zero-test-discovery-before-corrected-run")
        || text(value, "source_commit") != Some("6c107abfcec60160c9aec13b17d7f92bb71fc043")
        || text(value, "contract") != Some(W10_INTEGRATED_CANDIDATE_CONTRACT)
        || text(value, "candidate_identity") != Some("C73-provisional-1")
        || text(value, "candidate_source_commit")
            != Some("7e3070894aa51af96cdcb3e350eff923a309e1fa")
    {
        problems.push("W10 local qualification amendment identity changed".to_owned());
    }
    if boolean(value, "invalid_attempt_retained") != Some(true) {
        problems.push(
            "W10 local qualification amendment invalid_attempt_retained must be true".to_owned(),
        );
    }
    for field in [
        "invalid_attempt_passed",
        "threshold_changed",
        "candidate_changed",
        "product_changed",
        "other_commands_changed",
        "host_run_allowed",
        "local_results_promotable",
        "release_numerical_claim_allowed",
    ] {
        if boolean(value, field) != Some(false) {
            problems.push(format!(
                "W10 local qualification amendment {field} must be false"
            ));
        }
    }
    if integer(value, "invalid_attempt_test_count") != Some(0)
        || integer(value, "minimum_replacement_tests") != Some(5)
        || text(value, "replacement_command")
            != Some(
                "cargo test -p hydracache --features durable-value-store --test durable_value_store --locked",
            )
    {
        problems.push("W10 local qualification amendment correction changed".to_owned());
    }
    for field in ["trigger", "decision_rule", "next_decision"] {
        if text(value, field).is_none_or(str::is_empty) {
            problems.push(format!(
                "W10 local qualification amendment {field} is missing"
            ));
        }
    }
    problems
...
pub fn check_w10_static_qualification_amendment(value: &TomlValue, release: &str) -> Vec<String> {
    let mut problems = Vec::new();
    if integer(value, "schema_version") != Some(1)
        || text(value, "release") != Some(release)
        || text(value, "amendment_id") != Some("w10-static-qualification-amendment-a725077d-v1")
        || text(value, "state")
            != Some("preregistered-after-feature-gate-failure-before-corrected-run")
        || text(value, "source_commit") != Some("a725077dcabdab2f4f2cb6301ae528a6532dd81a")
        || text(value, "contract") != Some(W10_INTEGRATED_CANDIDATE_CONTRACT)
        || text(value, "candidate_identity") != Some("C73-provisional-1")
        || text(value, "candidate_source_commit")
            != Some("7e3070894aa51af96cdcb3e350eff923a309e1fa")
    {
        problems.push("W10 static qualification amendment identity changed".to_owned());
    }
    if boolean(value, "failed_attempt_retained") != Some(true) {
        problems.push(
            "W10 static qualification amendment failed_attempt_retained must be true".to_owned(),
        );
    }
    for field in [
        "failed_attempt_passed",
        "threshold_changed",
        "candidate_changed",
        "product_changed",
        "package_set_changed",
        ...
        if boolean(value, field) != Some(false) {
            problems.push(format!(
                "W10 static qualification amendment {field} must be false"
            ));
        }
    }
    if text(value, "replacement_command")
        != Some(
            "cargo clippy -p hydracache-client-transport-axum -p hydracache-core -p hydracache-redis-compat -p hydracache-server -p hydracache --all-targets --features hydracache/durable-value-store --locked -- -D warnings",
        )
    {
        problems.push("W10 static qualification amendment correction changed".to_owned());
    }
    for field in ["trigger", "decision_rule", "next_decision"] {
        if text(value, field).is_none_or(str::is_empty) {
            problems.push(format!(
...
pub fn check_w10_focused_host_tooling_evidence(value: &TomlValue, release: &str) -> Vec<String> {
    let mut problems = Vec::new();
    if integer(value, "schema_version") != Some(1)
        || text(value, "release") != Some(release)
        || text(value, "evidence_id") != Some("w10-focused-host-tooling-passed-514183a9-v1")
        || text(value, "state") != Some("local-tooling-qualified-host-dispatch-open")
        || text(value, "contract") != Some(W10_FOCUSED_HOST_COMPARISON_CONTRACT)
        || text(value, "tooling_implementation_commit")
            != Some("514183a9971d7f9cbb9b40d0c07acfb6e4a81ab5")
        || text(value, "tooling_implementation_tree")
            != Some("c283d24c5fb8bccd05e200b7a36b66a2d485fb20")
        || text(value, "harness_implementation_commit")
            != Some("c6e14fa9dbc969fa171470c8ef8ce49184fcd36f")
        || text(value, "dispatch_adapter_commit")
            != Some("9268536c3221be08460ac9cb3a40b3207ca6efad")
        || text(value, "dispatch_adapter_tree") != Some("e2561eeff61d7d7348eef0f210d0a914d97121c6")
        || text(value, "baseline_source_commit") != Some("e757556d3a31d565f52a9561d6d4e555bb1cc373")
        || text(value, "candidate_source_commit")
            != Some("7e3070894aa51af96cdcb3e350eff923a309e1fa")
        || text(value, "candidate_tree_oid") != Some("e2c438b9a248586a4f72d3eca3d1c5369fff440b")
    {
        problems.push("W10 focused host tooling evidence identity changed".to_owned());
    }
    for field in [
        "contract_sha256",
        "overlay_sha256",
        ...
        "runner_test_sha256",
        "workflow_sha256",
        "dispatch_entry_workflow_sha256",
        "candidate_receipt_sha256",
        "canary_packet_sha256",
        "local_harness_binary_sha256",
        "local_server_binary_sha256",
    ] {
        if text(value, field).is_none_or(|digest| !sha256(digest)) {
            problems.push(format!(
                "W10 focused host tooling evidence {field} is not SHA-256"
            ));
        }
    }
    for field in [
        "working_tree_dirty",
        "i73_product_tree_modified",
        "c73_product_tree_modified",
        "full_host_volume_reduction_allowed",
        ...
        if boolean(value, field) != Some(false) {
            problems.push(format!(
                "W10 focused host tooling evidence {field} must be false"
            ));
        }
    }
    for field in [
        "harness_clippy_warnings_denied",
        "candidate_local_process_passed",
        ...
        if boolean(value, field) != Some(true) {
            problems.push(format!(
                "W10 focused host tooling evidence {field} must be true"
            ));
        }
    }
    if integer(value, "runner_unit_tests_passed") != Some(6)
        || integer(value, "host_attempts_required") != Some(30)
        || integer(value, "local_operations") != Some(1_000)
        || integer(value, "local_warmup_operations") != Some(100)
        || integer(value, "local_success") != Some(1_000)
        || integer(value, "local_hc2_success") != Some(350)
        || integer(value, "local_resp_success") != Some(300)
        || integer(value, "local_hc1_success") != Some(150)
        || integer(value, "local_direct_success") != Some(100)
        || integer(value, "local_tag_success") != Some(50)
        || integer(value, "local_ttl_success") != Some(50)
        || integer(value, "local_events_received") != Some(350)
        || integer(value, "local_durable_success") != Some(1_000)
        || integer(value, "canary_exit_code") != Some(1)
        || text(value, "canary_marker") != Some("HC-CANARY-RED:W10-HOST")
    {
        problems.push("W10 focused host tooling evidence result changed".to_owned());
    }
    if value
        .get("invalid_attempt")
        .and_then(TomlValue::as_array)
        .is_none_or(|attempts| attempts.len() != 9)
    {
        problems.push("W10 focused host tooling evidence invalid attempts changed".to_owned());
    }
    for field in [
        "raw_candidate_receipt",
        "raw_canary_packet",
        "local_resource_reason",
        "host_allocation_diagnostic",
        "claim_boundary",
        "dispatch_rule",
        "next_evidence",
    ] {
        if text(value, field).is_none_or(str::is_empty) {
            problems.push(format!(
                "W10 focused host tooling evidence {field} is missing"
            ));
        }
    }
