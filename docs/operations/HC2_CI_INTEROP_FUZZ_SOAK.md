...
| `docker-interop`  | pull request, main push, tag, schedule, manual | GitHub `ubuntu-24.04` + pinned container | 90 min | production daemon mTLS/coexistence/startup/drain tests plus Rust spike tests, real Java fixture/installed SDK consumer, offline Python, and retained fault replay in one reproducible process environment; the bound includes image rebuild and transient package-mirror delay |
