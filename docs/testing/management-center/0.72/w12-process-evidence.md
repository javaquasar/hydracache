...
- both start three production daemons with RESP, poll the typed dashboard every second, issue HC/1
  ...
  binary-bound JSON artifact before evaluating those terminal ceilings. Receipt schema v3 records
  every live daemon's stable node index/id, current pid, RSS, process-lifetime HWM and FD count at
  60-second intervals, including pid changes after a follower restart;
...
The idle diagnostic profiles deliberately send no management, HC/1 or RESP requests and schedule no
fault. `diagnostic_single_node_idle_resource_slope` distinguishes process-local background work from
peer traffic; `diagnostic_idle_cluster_resource_slope` runs the production three-member topology.
They are Linux-only diagnostics under `HYDRACACHE_RUN_MANAGEMENT_RESOURCE_LINUX_072=1`, sample each
pid every 30 seconds, and must show a plateau after the bounded admission journal fills. They are
localization evidence, not substitutes for either release soak.
