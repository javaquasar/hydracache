...
- real loopback socket evidence: dedicated TCP, HTTP/2, and gRPC green;
- CA-signed TLS plus required client certificate: gRPC green; TLS parity and
  negative certificate cases for the other candidates pending;
...
| --- | --- | --- |
| Generation isolation before dispatch | Green for all candidates | Pending |
| Verified identity before negotiation/dispatch | Green as boundary model | gRPC mTLS green; negative/parity cases pending |
| 256 concurrent correlated invocations | Green | Green on generated gRPC mTLS stream; parity pending |
| Interleaved reply, heartbeat, event push | Green | All three frame types green on gRPC; reply/event green on other candidates |
| Bounded slow consumer with explicit gap | Green | Pending |
| Disconnect clears pending calls and registrations | Green | Green for loopback happy-path close |
...
| Current executable proof | Real loopback frame/reply | Real H2 request/response stream with pushed event | Generated bidi stream with CA-signed mTLS, 256 replies/heartbeat/event, Rust/Java golden frame |
...
The next W0 slice must add TLS parity and negative certificate tests, exercise
256 real-stream correlations plus heartbeat/cancellation/slow-consumer faults,
add Python generation, and record dependency/binary-size artifacts. Only then
can ADR-0019 become Accepted and W1 make the selected schema authoritative.
