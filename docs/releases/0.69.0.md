...
- versioned `READY_DAEMON_V1` process-fixture receipts shared by Rust producer/consumer code and
  asserted by Java consumers; this closes the first PR-run regression caused by a stale field count;
- fail-loud admission and per-lane status artifacts, an independent fast-evidence job, and explicit
  head/base/tested-SHA provenance with checkout equality enforcement;
