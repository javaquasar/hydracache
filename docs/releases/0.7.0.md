...
- `SqlxCache::cached::<T>()` for cache descriptors without duplicate SQL text.
- `SqlxCache::named::<T>("name")` for optional diagnostic labels.
...
`hydracache-sqlx` adds `sqlx 0.8`, which pulls URL/IDNA/ICU crates
transitively. The lockfile keeps that graph on ICU `2.1.x` because ICU `2.2.x`
requires Rust `1.86`, while HydraCache currently declares Rust `1.85` as MSRV.

This is tracked as technical debt in
