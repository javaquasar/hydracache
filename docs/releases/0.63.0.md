...
includes the narrow single-endpoint Redis lock subset needed by redis-py `Lock`
and Node `redlock` single-resource migrations. It does not make HydraCache a
Redis clone.
...
- Added protocol v4 lock-conditional coverage for the narrow Redis lock subset:
  ...
  single-resource API rows are in the Docker-gated matrix, and the release does
  not claim Redlock quorum, Redis Cluster, or general Lua compatibility.
...
- General Redis Lua remains unsupported. Only the reviewed lock-script allowlist
  is accepted for redis-py `Lock`, simple token-safe release/extend, and Node
  `redlock@5.0.0-beta.2` single-resource scripts. Unknown or changed scripts
  fail loud before mutation. Redisson full locks, Redlock quorum semantics,
  multi-key locks, transactions, modules, pub/sub, streams, and Redis data
...
release/extend scripts, redis-py `Lock`, Node `redlock` single-resource API,
