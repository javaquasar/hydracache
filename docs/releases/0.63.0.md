...
- Implemented supported cache-subset translation for `PING`, `ECHO`, `QUIT`,
  `HELLO 2`, `HELLO 3`, `COMMAND`, `SELECT 0`, `GET`, `SET`, `MGET`, `MSET`, `DEL`, and `EXISTS`.
...
`SELECT 0`, and the `rediss://` startup path, against real Redis after documented normalization.
