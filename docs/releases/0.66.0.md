...
Status: **shipped** as `v0.66.0` on 2026-07-21.
...
Release 0.66 was closed only after:

- the mixed-version ship proof built the previous daemon from the full-history annotated `v0.65.0`
  tag (peeling to `edf0fd1`) and recorded that provenance on the exact candidate instead of using
  the pinned `2926551` development fallback;
- operator kind/Chaos, bounded cargo-fuzz, and Linux process/resource receipts were produced by
  their registered release lanes on the exact candidate commit; and
- `--require-ship` accepted the clean candidate with every receipt bound to that commit and its
  registry digest.

Ship status was determined by
