...
| `SledRaftLogStore` snapshot envelope | `1` | `hydracache-cluster-raft` `sled-log-store` feature | New snapshot writes use a magic/version/length/checksum envelope around the raft protobuf payload. Readers still accept legacy raw protobuf snapshots for backward compatibility, but enveloped snapshots verify length and checksum before install. | Bit-flipped or truncated snapshot bytes fail loud during store open; valid snapshots restored into the wrong metadata identity are rejected by the runtime identity contract before apply. |
