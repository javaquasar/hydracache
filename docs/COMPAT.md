...
| HydraCache Redis RESP edge surface | `RESP2 subset v1` | `hydracache-redis-compat` and opt-in `hydracache-server` Redis listener lifecycle | `0.63.0` registers an off-by-default RESP2 cache-subset facade. It translates supported Redis cache commands into the stable HydraCache client protocol instead of changing that protocol. RESP3 is not claimed; `HELLO 3` and RESP3-only inputs fail loud or remain candidate as documented in `docs/integrations/redis_compat_conformance.json`. | Unsupported commands, admin-disabled commands, unknown RESP3-only frames, oversized frames, malformed/truncated frames, wrong tenant scope, and candidate TTL/tag commands fail loud before mutation. No `MOVED`/`ASK`/Redis Cluster redirects are emitted. |
