...
| HydraCache Redis RESP edge surface | `RESP2 subset v1` | `hydracache-redis-compat` and opt-in `hydracache-server` Redis listener lifecycle | `0.63.0` registers an off-by-default RESP2 cache-subset facade. It translates supported Redis cache commands into HydraCache client protocol operations. The expanded 0.63 scope depends on protocol v3 TTL metadata/expiry operations for `SET EX/PX`, `EXPIRE`/`PEXPIRE`, `PERSIST`, `TTL`, and `PTTL`, and on atomic client-surface batch apply for `MSET`. RESP3 is not claimed; `HELLO 3` and RESP3-only inputs fail loud or remain candidate as documented in `docs/integrations/redis_compat_conformance.json`. | Unsupported commands, admin-disabled commands, unknown RESP3-only frames, oversized frames, malformed/truncated frames, wrong tenant scope, candidate tag commands, non-atomic `MSET`, and missing TTL metadata fail loud before mutation. No `MOVED`/`ASK`/Redis Cluster redirects are emitted. |
