...
| HydraCache Redis RESP edge surface | `RESP2 subset v1` | `hydracache-redis-compat` and opt-in `hydracache-server` Redis listener lifecycle | `0.63.0` registers an off-by-default RESP2 cache-subset facade. It translates supported Redis cache commands into HydraCache client protocol operations. The expanded 0.63 scope depends on protocol v3 TTL metadata/expiry operations for `SET EX/PX`, `EXPIRE`/`PEXPIRE`, `PERSIST`, `TTL`, and `PTTL`, atomic client-surface batch apply for `MSET`, and Redis `AUTH`/`HELLO 2 AUTH` semantics for auth-required listeners. TLS/rediss posture is part of the same compatibility row: it is either backed by existing server TLS/mTLS material and tests, or documented unsupported for `0.63.0` with external TLS/private-network controls required for production. RESP3 is not claimed; `HELLO 3` and RESP3-only inputs fail loud or remain candidate as documented in `docs/integrations/redis_compat_conformance.json`. | Unsupported commands, admin-disabled commands, unknown RESP3-only frames, oversized frames, malformed/truncated frames, unauthenticated data commands, wrong tenant scope, candidate tag commands, non-atomic `MSET`, missing TTL metadata, and ambiguous `rediss://` behavior fail loud before mutation. No `MOVED`/`ASK`/Redis Cluster redirects are emitted. |
