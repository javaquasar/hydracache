...
| Server admin status JSON shape | `1` | `hydracache-server` `/admin/status` | `0.57.0` adds the required `source` field (`live` or `modeled`) to the existing operator status shape (`leader`, `term`, `quorum_ok`, `members`, `reshard_phase`, `draining`). Management Center consumers must carry this source tag through every rendered status so modeled values are never shown as live. Existing operator readers that ignore unknown fields remain compatible. | Missing, unknown, or untrusted `source` values must be treated as `modeled`/not-live by strict readers; consumers must fail loud or degrade honestly rather than painting modeled data as live. |
