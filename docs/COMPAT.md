...
W4 bumps the simulator snapshot JSON schema to version `5`. Topology controls
(`isolate`, `rejoin`, `disable`, `enable`, `add_node`) now execute through
`ControlActionV1`; snapshots expose disabled nodes and deterministic rebalance
progress. The W4 add-node/reshard path is a simulator model for the lab and does
not claim production ownership movement.
