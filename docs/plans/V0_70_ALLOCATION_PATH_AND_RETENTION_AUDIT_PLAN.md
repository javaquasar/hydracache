...
| W7 governance | implemented for fast CI | The release owns a fail-closed W0-W7 canary registry, a closure guard/canary test, registry-completeness coverage, and exact `canary-check`/fast-sweep wiring on the GitHub-hosted Rust job. The local diagnostic campaign and unresolved conditional-tombstone watermark remain explicit release blockers. |
