...
0.37 DB production hardening ──► 0.38 DB correctness automation
   ...
   0.66 ─┄ feeds ┄► 0.67 Performance Measurement Infrastructure
                          (W0-W10 open-loop loadgen, surface-scoped scenarios, report schemas,
                          prebuild identity, immutable-anchor/rolling-baseline validation,
                          canaries, exact receipts, hosted tripwire, and protected reference
                          workflow. The release ships tooling and methodology only. Official
                          capacity, sizing, Redis-comparison, metrics-agreement, and numerical
   ...
   0.67.0 ─┄ feeds ┄► 0.68 Generated Client Plane Foundation
                          ...
                          retry, cancellation, server-push listener, topology, and CP-session
                           machinery; publish the Rust HC/2 crate while retaining generated
                           Java/Python clients and the Hazelcast-shaped Java facade as tested
                           source previews. Legacy HC/1 v1-v4 readers remain intact;
   ...
   0.71 ─┄ feeds ┄► 0.72 Management Center 2.0 & Operational Visibility
                          (expose the measured, bounded operational state through a versioned
...
> **0.67 shipped status (2026-07-27):** W0-W10 measurement infrastructure and methodology are
> shipped without numerical claims. Dedicated `reference-v1` bootstrap and final reference evidence
> remain deferred to TD-0013; the profile is unbootstrapped and hosted results stay tripwire-only.
...
| [0.66.0](V0_66_CLUSTER_REAL_PROCESS_AND_OPERATIONAL_HARDENING_PLAN.md) | shipped | **Cluster Corner-Case Hardening - Real-Process & Operational Tier.** W0 exposes the existing typed Sled compaction path; W1 covers real snapshot catch-up plus sender/receiver death; W2/W3/W7 prove only consensus-backed control-plane history, preserving the 0.65 node-local client boundary; W4 makes the currently unwired live backup/restore boundary executable; W5 covers snapshot/commit IO faults; W6 runs mixed old/new daemon binaries while consuming 0.64 W32 byte fixtures; W8 adds differential metadata-model checks; W9 extends the existing fuzz workspace and adds a socket corpus; W10 covers scheduler/tick perturbation without lease-read claims; W11 covers operator scale chaos; W12 proves snapshot sender resource release; W13 supplies release-scoped canaries, exact-command receipts, and `--require-ship` governance. No distributed client backend, ownership routing, live backup engine, encrypted live restore, or lease-read API is claimed. | Lift the shipped in-process Raft/Sled proofs to real daemons without contradicting the executable node-local claims added by 0.65, and bind every process/operator/fuzz guarantee to replayable evidence from the exact candidate commit. | 0.65.0 | 1.0 |

| [0.67.0](V0_67_PERFORMANCE_CHARACTERIZATION_PLAN.md) | shipped | **Performance Measurement Infrastructure** - W0-W10 coordinated-omission-safe open-loop load generation, surface-scoped scenarios, typed reports, runner attestation, prebuilt-binary binding, immutable-anchor/rolling-baseline validation, canaries, exact receipts, hosted tripwires, and a protected manual reference workflow. No product surface or optimization is introduced. Official capacity, sizing, Redis-comparison, metrics-agreement, and numerical baseline evidence is deferred to TD-0013; `reference-v1` remains unbootstrapped and its five fail-closed reference gates are not 0.67 ship-mandatory receipts. | Ship honest reusable measurement infrastructure now without manufacturing results on shared hardware; bootstrap dedicated reference evidence later under explicit debt and independent review. | 0.66.0 | 1.0 |

| [0.67.1](V0_67_1_DEDICATED_PERFORMANCE_REFERENCE_BOOTSTRAP_PLAN.md) | shipped | **Dedicated Performance Reference Bootstrap** - one bare-metal fingerprint passed qualification and two-run full-dress admission; exactly five chained bootstrap samples were retained; the anchor, rolling baseline, and budgets were independently reviewed and activated; W7's separate exact-SHA frozen candidate passed with W0-W7 `ship-ready`; and the anonymized report, methodology article, and byte-exact raw campaign archive were published. | Preserve the strict reference method and candidate self-baseline prohibition. No failed run, optimization, product surface, portable sizing statement, or universal comparison is promoted. | 0.67.0 | 1.0 |
...
| [0.70.0](V0_70_ALLOCATION_PATH_AND_RETENTION_AUDIT_PLAN.md) | shipped | **Allocation Path & Retention Audit** — inventory every long-lived allocation owner, add deterministic retained-state snapshots and phase-aware allocation tests, prove cleanup for tag generations, load breakers, client histories, cache maintenance and HC/2 resources, fix confirmed unbounded owners, and provide a local non-ship Hydra/Redis runner with verified native reset. | RSS alone cannot distinguish live state, tombstones, queues, allocator high-water, page cache, or a cleanup command that never succeeded. Establish causal owner evidence before optimizing the process footprint. | 0.69.0 | 0.71 |
| [0.71.0](V0_71_MEMORY_FOOTPRINT_AND_RETENTION_EFFICIENCY_PLAN.md) | shipped | **Memory Footprint & Retention Efficiency** — consume the 0.70 owner inventory and completed 0.67.1 dedicated-reference bootstrap; keep the unmodified 0.70 release baseline separate from the pre-optimization instrumented 0.71 baseline; correct byte accounting without silently reinterpreting legacy capacity; compact only measured key/entry/tag structures; reduce copies; compare allocators and optional profiles; distinguish durable page cache; and pass a finite fixed-cardinality 60-minute/six-hour/24-hour matrix. | Turn causally classified owners into measured optimizations without lowering capacity/workload or weakening correctness, SLO, zero-error, privacy, compatibility or fail-closed behavior. Dedicated-host admission is an explicit D0/D4/ship prerequisite; deterministic/source work may be prepared while it is unavailable. A fully evidenced no-win result may ship with accounting/bounds claims and no numerical improvement claim. | 0.70.0, 0.67.1 | 1.0 |
| [0.72.0](V0_72_MANAGEMENT_CENTER_2_OPERATIONAL_VISIBILITY_PLAN.md) | planned | **Management Center 2.0 & Operational Visibility** — introduce a locally bundled TypeScript/component console and versioned read-only `/management/v1` API with dashboard, member/partition, HC/1-HC/2-RESP client, namespace/cache, health, persistence/operation/audit and bounded-history views. Server-side aggregation is authenticated and bounded; every observation is explicit about live/modeled/unavailable, freshness and completeness; optional Prometheus history accepts only fixed reviewed queries. | Turn HydraCache's existing operational signals into a rich Hazelcast-class management experience without copying unrelated entities or manufacturing cluster truth. Every new route, UI state, permission, bound and failure path is backed by unit/property/browser/security/real-process/fault/soak/upgrade tests, red canaries and exact-candidate evidence; the browser remains unable to mutate cluster state. | 0.71.0 | 1.0 |
| [0.73.0](V0_73_EVIDENCE_DRIVEN_PERFORMANCE_EFFICIENCY_PLAN.md) | planned | **Evidence-Driven Performance Efficiency** — use the accepted 0.71 negative result and the eventual published 0.72 artifact to freeze causal owner/stack baselines, then conditionally qualify shared-store/expiry, tag-index, RESP copy/key, HC/2 connection, optional-service/management, durable/page-cache, allocator and opt-in retained-byte-admission proposals. Each candidate has semantic and compatibility tests, falsifying canaries, paired same-host measurements and exact-SHA release evidence; a no-win result remains publishable without a numerical claim. | Improve per-owner/per-connection memory, allocation/copy cost and tail latency only where a preregistered proposal proves a practical gain without changing legacy semantics, correctness, security or supported targets. The 0.71 D4 run did not prove an RSS win, and 0.72 is not yet a published numerical baseline. | 0.72.0 | 1.0 |
