...
0.37 DB production hardening ──► 0.38 DB correctness automation
   ...
   0.57 (+cluster-raft/chitchat/transport-axum) ─┄ feeds ┄► 0.59 Networked Daemon Grid
                          Hosting (close TD-0008 / 0.57 W6b — the #1 maturity gap to 1.0:
                          the deployable daemon actually hosts the real networked grid in
                          member role. Wire the SHIPPED adapters (RaftMetadataRuntime as
                          ClusterControlPlane over storage_dir, ChitchatDiscovery over
                          seeds, cluster-transport-axum on cluster_addr) into grid_host.rs;
                          expose raft leader so /cluster/overview leader is no longer null;
                          loopback 3-daemon E2E: form cluster, elect one leader, kill-leader
                          reelect without stale leader; TLS-bound; integration not new
                          consensus; flips source:live to true multi-node; -> enables a
                          defensible 1.0 "cluster out of the box")
```
...
| [0.58.0](V0_58_ENDURANCE_SOAK_AND_OVERLOAD_HARDENING_PLAN.md) | planned | **Endurance — Soak & Overload Hardening** — turn the single-shot **VOPR** (`hydracache-sim/src/bin/vopr.rs`) into a **continuous, wall-clock-budgeted, multi-seed soak driver** (first failing seed replays exactly, R-5; stops loud on violation, R-3); **resource-leak-over-time** invariants (falsifiable bounded-growth over SimStorage bytes / in-flight / subscriber buffers / tombstone debt) + real-server RSS/fd sampler; **sustained-overload / backpressure** proof + hardening over the shipped admission/capacity path (rejects counted, queues bounded, no OOM, recovers-after); a **real multi-node chaos soak** on the 0.56 operator/kind harness (no lost committed write, skip-gracefully); **bounded CI soak gate + extended nightly + `SOAK_REPORT`** | Close the most honest remaining weakness — **no soak mileage / unproven under sustained overload** (develop-**downward**); the algorithms are validated, **endurance** is not — no new algorithms, no throughput/self-score claim (R-7), no new consistency level (R-1), fast path unchanged (R-10), soak is evidence not a battle-tested claim (R-11) | 0.44, 0.46, 0.56, 0.57.1 | — |
| [0.59.0](V0_59_NETWORKED_DAEMON_GRID_HOSTING_PLAN.md) | planned | **Networked Daemon Grid Hosting** — make the deployable `hydracache-server` **actually host the real networked grid** in member role, closing 0.57 W6b / **TD-0008** (the #1 maturity gap to 1.0). Today `grid_host.rs` builds an **in-process** control plane and `raft_leader_id()` returns `None` (grid_host.rs:154) → `/cluster/overview` `leader:null`. **W1** expose `leader_id` from raft soft-state; **W2** wire the shipped adapters — `RaftMetadataRuntime` (durable, over `storage_dir`) as `ClusterControlPlane`, `ChitchatDiscovery` (chitchat:404) over `seeds`, `hydracache-cluster-transport-axum` on `cluster_addr`; **W3** `NetworkedGridHandle` (real quorum/leader/reachability/drain-leaves-raft-cleanly); **W4** TLS-bound cluster listener + fail-loud; **W5** loopback **3-daemon E2E** enabling the ignored sentinel (form cluster → elect one leader → kill-leader → reelect without stale leader; network-gated, skip-graceful); **W6** runbook + TD-0008 Resolved | Close the deployable-artifact gap: `0.42`–`0.56` proved the grid in library/DST/transport tests, but the daemon runs `HydraCache::local()` — **integration, not new consensus** (R-1); flips `source:live` from single-in-process-member to true multi-node; `local`/`client` stay `modeled`; fast path unchanged (R-10); prerequisite for a `1.0` "cluster out of the box" claim | 0.57.0 | 1.0 |
