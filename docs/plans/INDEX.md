...
0.37 DB production hardening ──► 0.38 DB correctness automation
   ...
   0.57 + 0.58 (+cluster-raft/chitchat/transport-axum) ─┄ feeds ┄► 0.59 Networked Daemon Grid
                          ...
                          loopback 3-daemon E2E uses real ServerRuntime members; TLS-bound;
...
| [0.59.0](V0_59_NETWORKED_DAEMON_GRID_HOSTING_PLAN.md) | planned | **Networked Daemon Grid Hosting** — make the deployable `hydracache-server` **actually host the real networked grid** in member role, closing 0.57 W6b / **TD-0008** (the #1 maturity gap to 1.0). Today `grid_host.rs` builds an **in-process** member control plane and `raft_leader_id()` returns `None` (grid_host.rs:154) → `/cluster/overview` `leader:null`. **W1** expose `leader_id`; **W1b** make `RaftMetadataRuntime` network-drivable and multi-voter (stable node-id mapping, tick/step/drain/outbound `RaftWireMessage`, voter/conf-change path, HTTP sink/handler); **W2** wire `ChitchatDiscovery`, the same durable/networked `RaftMetadataRuntime` as `HydraCache::member()` `ClusterControlPlane`, `ClusterAdmissionBridge`, and `hydracache-cluster-transport-axum` into `grid_host.rs`; **W3** `NetworkedGridHandle` reads the same runtime for real quorum/leader/reachability/drain; **W4** TLS-bound cluster listener + fail-loud; **W5** loopback **3-daemon E2E** over real `ServerRuntime` members; **W6** runbook + TD-0008 Resolved + 0.58 soak re-point | Close the deployable-artifact gap: `0.42`–`0.56` proved the grid in library/DST/transport tests, but the member-role daemon is still in-process-only, not a true multi-daemon grid — **integration around shipped raft-rs, not new consensus** (R-1); flips `source:live` from single-in-process-member to true multi-node; `local`/`client` stay `modeled`; fast path unchanged (R-10); prerequisite for a `1.0` "cluster out of the box" claim | 0.57.0, 0.58.0 | 1.0 |
