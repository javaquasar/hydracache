...
| [0.67.1](V0_67_1_DEDICATED_PERFORMANCE_REFERENCE_BOOTSTRAP_PLAN.md) | in-progress | **Dedicated Performance Reference Bootstrap** - implementation, attestation, orchestration, review, and frozen-candidate machinery are complete; qualifying bare-metal acquisition and the five accepted bootstrap samples are deferred. No failed run, optimization, product surface, or numerical claim is promoted. | Preserve the strict reference-v1 method while suitable hardware is unavailable; the campaign remains open without blocking source releases that make no capacity claim. | 0.67.0 | 1.0 |
