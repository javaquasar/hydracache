...
0.37 DB production hardening ──► 0.38 DB correctness automation
   ...
   0.62.1 ─┄ feeds ┄► 0.63 Redis RESP Edge Facade (OUTWARD adoption: optional,
                          ...
                          reuses ClientSurfaceState (tenancy/limits/consistency); W0 executable
                          conformance manifest decides the supported subset before W2/W3; minimum
                          GET/SET/MGET/DEL + startup handshake, Redis counts/order semantics,
                          TTL only if the metadata gate lands, HC.* native-or-unsupported;
                          loud ERR-unsupported for everything else; NO MOVED/ASK/Cluster
                          ...
                          = anti-references; not a Redis clone; edge crate, core+stable client
...
| [0.63.0](V0_63_REDIS_RESP_EDGE_FACADE_PLAN.md) | planned | **Redis RESP Edge Facade** — an optional, **off-by-default** edge server mode (new `hydracache-redis-compat` crate + own listener, default `:6379`) speaking the **Redis RESP** protocol for the **cache subset**, so existing Redis clients point at HydraCache by changing a connection string. **Translates** RESP into the shipped `ClientRequest`/`ClientResponse` family (client-protocol lib.rs:612/795) and **reuses** the `ClientSurfaceState` execution layer (client-transport-axum lib.rs:283) for tenancy/limits/consistency — no cache-access re-implementation, no change to the stable `hydracache-client-protocol` (R-4). W0 adds a versioned conformance manifest and real-Redis oracle normalization before commands are claimed; minimum target is `GET/SET/MGET/DEL` + startup handshake with Redis counts/order semantics; `TTL` only ships if the metadata gate lands; `MSET` is atomic or unsupported-loud; **`HC.*`** is native-or-unsupported, with no scan-and-loop fake tag invalidation; **loud** `ERR unsupported` for everything else; **no** `MOVED`/`ASK`/Cluster (authority stays raft+epoch, R-1); golden RESP fixtures + pinned Docker `redis-server` oracle + Docker-gated multi-language client smoke + decode fuzz + pipelining/reconnect/resource/multi-node checks | Close the named POSITIONING adoption gap (non-Rust reach) with the highest-leverage **outward** step — "change the connection string, not the code"; an **edge crate** that never touches the frozen core, so compatible with a later `1.0` freeze; Redis Cluster/async-replication are **anti-references**; not a Redis clone, not an event log (R-9); off by default, fast path unchanged (R-10) | 0.62.1 | — |
