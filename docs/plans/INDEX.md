...
- [`V0_DRAFT_KUBERNETES_OPERATOR_PLAN.md`](V0_DRAFT_KUBERNETES_OPERATOR_PLAN.md) —
  **DRAFT (version TBD, candidate 0.56)**. Idea-capture for a HydraCache **Kubernetes Operator**: a
  `HydraCacheCluster` CRD + reconcile controller for the full lifecycle (install, scale with 0.43
  online reshard + drain, zero-downtime rolling upgrade via 0.48 graceful upgrade, cert/key rotation
  via 0.48 mTLS, persistence volumes via 0.51, scheduled backup/PITR via 0.48, health/admission +
  least-privilege RBAC). Orchestration over shipped 0.42–0.51 primitives, not new core machinery —
  closes the named Hazelcast Platform Operator gap (develop-downward). Tooling decision (kube-rs vs
  Go) at promotion. Builds on 0.48.
