...
0.37 DB production hardening ──► 0.38 DB correctness automation
   ...
   0.64 + 0.65 ─┄ feed ┄► 0.66 Cluster Corner-Case Hardening - Real-Process &
                          Operational Tier (lift the 0.64 IN-PROCESS raft/snapshot corner-case
                          proofs to REAL multi-daemon processes + real disk + backup/operator
                          chaos, AND add a widened external/adversarial tier, modeled on qdrant
                          consensus_tests / tikv test_raftstore / ScyllaDB randomized_nemesis /
                          TigerBeetle vortex+storage+time; W0 the one production change: a
                          disk-backed compaction seam on the server raft log store (off by default)
                          that 0.64 lacked to test real InstallSnapshot; W1 closes the 0.64 W10
                          deferral (real-process rejoin-after-compaction via InstallSnapshot); W2
                          nemesis+linearizability over the real client surface; W3
                          membership-under-partition+load loses no committed write; W4 backup/PITR
                          during live ops; W5 slow-disk/IO chaos; W6 rolling upgrade under format
                          drift; W7 external wire-only Jepsen linearizability harness; W8
                          metamorphic/differential vs reference model; W9 wire-boundary fuzzing;
                          W10 process-tier clock skew (lifts 0.64 W14); W11 operator scale chaos;
                          W12 encrypted-backup key-rotation restore; W13 local + GitHub CI
                          (daemon/kind/fuzz lanes). Seeded/replayable, fail-loud, paired canaries;
                          only production change is the W0 seam; no new consensus/consistency/routing)

   0.66 ─┄ feeds ┄► 0.67 Performance Characterization & Capacity Baselines
                          (measurement-only: answer "how much load does the embedded cache, a bare
                          daemon, and the cluster hold at a declared SLO" with recorded artifacts;
                          ...
                          scaling/contention/eviction/allocation curves (Caffeine JMH); W2 bare-node
                          native-protocol knee + connection/payload sweeps (YCSB mixes); W3 RESP tier
                          via real redis-benchmark against the 0.63 facade; W4 cluster knee at 3/5/7
                          nodes + consistency-cost matrix + replication write amplification;
                          W5 brownout depth/recovery for failover/membership/rebalance/kill;
                          W6 overload goodput curves at 1.2-2x knee; W7 macro perf budgets +
                          perf-budget-check (no silent rebaseline); W8 same-box comparative vs real
                          ...
                          No optimization work, no product surface; loadgen is a dev-only crate)
...
| [0.66.0](V0_66_CLUSTER_REAL_PROCESS_AND_OPERATIONAL_HARDENING_PLAN.md) | planned | **Cluster Corner-Case Hardening - Real-Process & Operational Tier** - lift the `0.64` **in-process** raft/snapshot corner-case proofs to the **real multi-daemon process tier** and close the operational corner cases the reference process-cluster harnesses (qdrant `consensus_tests`, tikv `test_raftstore`) test but HydraCache only covers in-process. **W0** the one load-bearing production change: a disk-backed log/snapshot **compaction seam** on the server raft log store (off by default, inert unless enabled) - the exact seam `0.64` said it lacked to test real `InstallSnapshot`. **W1** closes the named `0.64` W10 deferral (real-process rejoin-after-compaction caught up via `InstallSnapshot` over real disk + leader restart mid-install). **W2** lifts the `0.64` composed-fault nemesis to real daemons with linearizability over the real client surface. **W3** membership change under asymmetric partition + sustained load loses no committed write / no split-brain / idempotent retry coalescing. **W4** backup/restore/PITR during live ops and across a membership change, resurrecting no fenced data. **W5** slow-disk/IO chaos at the process + kind/operator tier degrades loud and safe. **W6** rolling upgrade under snapshot/wire format drift keeps golden vectors bidirectionally decodable. Widened external/adversarial tier: **W7** external wire-only Jepsen-style linearizability harness (`hydracache-jepsen`, drives real daemons over the public wire, checks history offline); **W8** metamorphic/differential testing vs an independent reference model (prefix-replay idempotence, independent-op commutativity, snapshot-then-tail equivalence); **W9** wire-boundary fuzzing of the raft HTTP transport (cargo-fuzz + deterministic corpus, never panics/mutates, bounds allocation); **W10** process-tier clock skew/backward jump over real daemons (lifts `0.64` W14 from the sim); **W11** operator-tier scale chaos on kind (`spec.replicas` churn under partition, no ghost voter); **W12** encrypted-backup key-rotation restore into a live cluster (fails loud on wrong key, no plaintext leak). **W13** local + GitHub CI (fast compile + W8/W9/W7-canary on PR; scheduled `Cluster Process Corner-Case Nightly` with daemon/kind/fuzz lanes + artifacts). Seeded/replayable (R-5), fail-loud (R-3), paired canaries; gated tiers green only when their require flag runs them | `0.64` proved the snapshot/membership corner-case grid **in-process**; the reference process harnesses (qdrant/tikv/ScyllaDB/TigerBeetle) show a distinct class only appears once real `hydracache-server` processes, real disk, real backup/restore, real wire, and operator chaos are in play - rejoin over compacted logs, partition-under-load, PITR during membership change, IO chaos, upgrade-under-drift, external adversarial linearizability, wire fuzzing, clock skew, scale chaos. `0.66` makes that class mechanically testable end-to-end and closes the last named `0.64` deferral; the only production change is the W0 compaction seam | 0.65.0 | 1.0 |

| [0.67.0](V0_67_PERFORMANCE_CHARACTERIZATION_PLAN.md) | planned | **Performance Characterization & Capacity Baselines** - a **measurement-only** release answering, with recorded artifacts, how much load the **embedded cache**, a **bare daemon**, and the **cluster** actually hold at a declared SLO. **W0** the instrument: fixed-rate **open-loop** loadgen where a missed tick counts as latency (wrk2/HdrHistogram, Gil Tene - coordinated-omission-safe), stepped-rate **knee search** (capacity = throughput at p99<=SLO, not peak), `PERF_REPORT` schema (hardware profile/commit/seed/spread), and the load-bearing canary proving a closed-loop recorder hides a stall the open-loop one reports. **W1** local-cache tier (Caffeine JMH blueprint): 1..N thread scaling per read/write mix, hot-key contention floor, eviction-pressure + allocation-per-op profiles. **W2** bare node over the native protocol (YCSB A/B/C): knee at SLO, connection scaling 1..1000, payload sweep, admission rejection overhead. **W3** bare node over RESP via **real `redis-benchmark`** against the `0.63` facade (ecosystem tooling runs unmodified), pipeline sweep, native-vs-RESP overhead ratio - closed-loop tool output recorded as throughput evidence only, never conflated with open-loop tail claims. **W4** cluster tier: knee at 3/5/7 daemons + scaling efficiency (USL fit, no extrapolation), consistency-level cost matrix incl. session/RYOW, replication write amplification, invalidation-fanout cost. **W5** operational **brownout profiles** - measured dip depth + recovery time for leader failover, membership add/drain, rebalance, kill/rejoin (`0.62`/`0.66` prove correctness; W5 measures the **cost**). **W6** overload goodput/recovery curves at 1.2x/1.5x/2x knee with an admission-bypass canary proving the curve reflects the mechanism. **W7** **macro perf budgets** keyed by hardware profile + `xtask perf-budget-check` (floor breach, unstable spread, silent rebaseline all fail - `0.37` bench-budget extended to the macro tier). **W8** same-box comparative vs pinned real Redis (artifact + methodology, not marketing, R-7). **W9** metrics honesty: server-reported ops/latency/rejects cross-checked against the independent observer. **W10** governance receipts (`release-evidence/0.67` + `--require-ship`), registry/canary rows, `Performance Nightly` CI lane, `docs/PERFORMANCE.md` | Today the repo proves the cluster **does not fall over** under load (`0.58`) but cannot answer **how much** the cache/node/cluster hold at an SLO: no open-loop generator, no coordinated-omission handling (0 mentions), no saturation knee, no scaling/consistency-cost/brownout numbers - any quoted p99 is methodologically suspect. `0.67` builds the instrument, records the numbers with hardware profiles, freezes them as CI-defended budgets, and bounds the honest Redis-comparative statement | 0.66.0 | 1.0 |
