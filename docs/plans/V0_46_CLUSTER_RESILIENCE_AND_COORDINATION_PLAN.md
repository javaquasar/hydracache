# HydraCache 0.46.0 Cluster Resilience & Coordination Plan
...
`0.46.0` deepens the **cluster itself**. Releases `0.42`–`0.45` built the shape of a
...
transport (`0.45`). What those releases did *not* yet adopt are several
...
for a single-key linearizable decision. `0.46` adds exactly those primitives —
...
numeric self-score. `0.46` does **not** weaken any `0.45` guarantee: every new
mechanism is opt-in or strictly strengthens an existing default, and embedded /
single-region deployments keep `0.45` behavior byte-for-byte.
...
the `0.41`–`0.45` artifact it strengthens. The work is six items (W1–W6) plus
explicit deferrals.
...
- **No new consistency claim beyond what each level delivers.** Tunable levels (W1)
  ...
  `0.45` bounded-staleness reality.
...
- **No KMS / secret-store, no ecosystem/external-consumer surface, no causal+
  ...
  autoscaler controllers.** These stay deferred (see Deferred To 0.47+).

## Inherited Boundary From 0.45

`0.46` only strengthens `0.41`–`0.45`; it must not redesign them.

- **`ReplicationConfig` read/write quorum (`0.41` A3) + grid RYOW (`0.42` W5) +
  LOCAL/EACH quorum intuition (`0.45`)** are generalized into **per-operation tunable
...
- **Anti-entropy / repair-gated GC (`0.41` A5, `0.42` W3/W6) and the WAN digest
  exchange (`0.45` W3)** are the baseline that **hinted handoff (W2)** complements
...
W2 + 0.45 W3 digest exchange ────────► W3 incremental Merkle repair (Scylla/Cassandra)
...
caller's region, ties `0.43`/`0.45`), `Quorum` (grid-wide majority), `EachQuorum`
...
weaker. Cross-region levels inherit the `0.45` bounded-staleness reality (no level
...
- `level_required_acks_matches_replica_math` (unit): `One`/`Quorum`/`All` over a
  known map.
- `local_quorum_counts_only_local_region` (unit): ties `0.45` regions.
...
**Problem / motivation.** `0.41`/`0.42` anti-entropy and the `0.45` W3 WAN digest
...
exchange only the differing ranges (generalizing the `0.45` W3 digest). Two modes:
...
2. Implement `RepairSession::run` using `diff` to exchange only mismatching ranges
   (supersedes the `0.45` W3 ad-hoc digest with a tree).
...
- `remote_client_replays_like_embedded` (**property**): ties B1 / `0.45`.
...
lossy/metered WAN (`0.45`).

`0.46` **adds** faults that exercise the new resilience primitives:
...
`0.46.0` may claim **resilient, coordinated cluster** (Hazelcast/ScyllaDB primitives
