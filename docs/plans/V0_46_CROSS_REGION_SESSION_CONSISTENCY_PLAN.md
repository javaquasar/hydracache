...
(`0.42` W5) and eventual / bounded-staleness **across** regions (`0.44`), plus
per-operation consistency levels (`0.45` W1). What sits between — the guarantees an
application *session* actually needs when its requests bounce between regions and
replicas — was missing. `0.46` adds **causal+ session consistency**: the four session
...
numeric self-score. `0.46` does **not** weaken any `0.45` guarantee: session
consistency is opt-in per session, sessionless callers keep `0.45` behavior
...
- **No full distributed transactions.** Serializable cross-node/cross-region
  ...
  The `0.43` W5 single-partition atomic-invalidation slice and the `0.45` W5 single-key
...
- **No remote code execution, no KMS, no ecosystem/external-consumer surface, no auto
  ...
  (see Deferred To 0.47+).

## Inherited Boundary From 0.45

`0.46` only extends `0.41`–`0.45`; it must not redesign them.

- **The hybrid logical clock (`0.44` W1) + `(version, epoch)` stamps (A5)** are the
  ordering substrate; `0.46` adds the **session watermark** built on them (W1).
- **Per-operation consistency levels (`0.45` W1)** are the enforcement lever: a session
  guarantee is satisfied by escalating the read level or waiting/repairing until the
  session watermark is met (W2/W3).
- **Active-active + bounded staleness (`0.44` W1/W6)** are the cross-region reality the
  session guarantees layer on; the staleness bound (`0.44` W6 SLO) becomes a per-read
  `BoundedStaleness` option in W5.
- **CRDT convergence + `MergePolicy` (`0.44` W2 / `0.42` W4)** provide the "+"
  (convergence to a single last value) in W5.
- **Read-repair + Merkle repair (`0.45` W3)** are how a lagging replica is brought up to
  a session's required watermark when a read would otherwise violate a guarantee.
- **Region failover (`0.44` W4)** is what session lifecycle (W6) must survive by
  reconstructing watermarks.
- **Near-cache watermark (`0.41` B1) + invalidation ring (`0.45` W6)** keep a session's
  near-cache consistent with its guarantees.

## Dependency Graph

```
0.44 HLC + A5 versions ──────────────► W1 session context & watermark
W1 + 0.45 W1 levels + 0.44 active-active► W2 read-your-writes (session, cross-region)
...
0.44 W2 CRDT + 0.42 W4 merge + 0.44 W6 ► W5 convergence + bounded staleness (the "+")
0.44 W4 failover + 0.42 W7 / 0.44 W6 ─► W6 session lifecycle, failover & observability
...
cannot be forged or replayed across sessions), and carried by the `0.45` W6 / `0.41` B1
client path. Sessionless requests omit the token and behave exactly as `0.45`.
...
1. Add `SessionId`, `VersionStamp` (reuse `0.44` HLC + A5 `(version, epoch)`),
...
4. Thread the token through the `0.45` W6 / `0.41` B1 client path and the cluster routes;
...
- `sessionless_path_is_unchanged` (integration): equals `0.45` behavior.
...
**Problem / motivation.** Under active-active (`0.44`), a session that writes in region
...
authority; `0.46` extends it to *follow the session* across regions.
...
another replica at a higher consistency level (`0.45` W1), or trigger a foreground
read-repair (`0.45` W3) / wait within a bounded budget, until the watermark is satisfied
...
1. On a session read, fetch the candidate replica stamp; if it `covers` the watermark,
   serve.
2. Otherwise escalate: higher CL (`0.45` W1) → foreground read-repair (`0.45` W3) →
...
- `write_region_a_read_region_b_sees_own_write` (integration): ties `0.44` active-active.
- `stale_replica_triggers_escalation_then_repair` (integration): ties `0.45` W1/W3.
...
- `ryw_holds_across_region_failover` (**chaos**, `#[ignore]`): ties `0.44` W4.
...
escalation ladder reuses `0.45` machinery; fail-loud beats serving stale.
...
only after it has applied all dependencies (or repaired up to them via `0.45` W3). The
...
3. On a replica, `causal_apply`: apply only when all dependencies are applied; otherwise
   defer and pull missing via `0.45` W3 repair, then apply.
...
(non-causally-ordered) writes converge via the `0.44` W2 CRDT merge for CRDT classes and
...
(tied to the `0.44` W6 staleness SLO), serving fast from the nearest replica when within
...
pub fn within_bound(wm: &SessionWatermark, pk: PartitionKey, replica: VersionStamp, max: StalenessBound) -> bool {
    // staleness measured in version/HLC distance, capped by 0.44 W6 SLO; never below a causal dep
...
1. Wire convergence: ensure concurrent writes for a key always reduce to one value via
   CRDT merge (`0.44` W2) or `MergePolicy` (`0.42` W4); add a convergence test that
...
3. Tie `max` to the `0.44` W6 staleness SLO units; expose the chosen bound per read.
...
that still honors causality; reuses `0.42`/`0.44` convergence.
...
failover (`0.44` W4), and be observable without exploding metric cardinality. A causal+
...
rebuilt). On region failover (`0.44` W4), a session's guarantees are preserved by
reconstructing the watermark against the promoted region (the watermark is client-carried,
so it survives the server-side promotion; the new region repairs up to it via `0.45` W3).
...
snapshot / audit (`0.45`-style). Governance events (token rejection, guarantee-unmet)
are audited.
...
1. Add `SessionTtl`; reject expired tokens (client rebuilds; degrade to sessionless
   meanwhile).
2. Preserve guarantees across `0.44` W4 failover: client-carried watermark + new-region
   repair (`0.45` W3) up to it; assert no guarantee regression.
...
4. Audit token rejections and guarantee-unmet events (`0.45`-style `AuditSink` if
   present).
...
- `guarantees_survive_region_failover` (**chaos**, `#[ignore]`): ties `0.44` W4 + `0.45`
  W3.
...
`0.46` reuses the `0.41`–`0.45` shared fault model and harness verbatim
...
(`0.44`), and liveness flapping / outage-window / lock pause-resume (`0.45`).

`0.46` **adds** faults that target session consistency:
...
`0.46.0` may claim **causal+ cross-region session consistency** only if **all** of the
...
- The fault model adds token replay/forgery, causal-metadata overflow, session migration,
  and causal-anomaly injection, and all those suites pass.
- Docs keep the prominent **"still not distributed transactions"** warning, state plainly
  that causal+ is **not** cross-region linearizable, and list ecosystem/external
  consumers, auto home-placement, and provider-specific autoscaler controllers as
  deferred to 0.47+.

If any condition fails, `0.46.0` ships **without** the corresponding claim, documents
