# HydraCache 0.45.0 Active-Active Multi-Region Plan
...
`0.45.0` builds on the zone/region-aware, elastically-resizable grid that
...
single write-authority region — `0.45` takes the grid **active-active across
...
no numeric self-score. `0.45` does **not** weaken any `0.43` guarantee: every new
...
- **No full distributed transactions.** Serializable cross-node / cross-region
  ...
  non-goal across the whole project. `0.45` adds no transaction semantics; the
...
`0.45` only extends `0.43`; it must not redesign it.

- **Zone/region-aware placement (0.43 W1)** placed replicas across zones within one
  write-authority region. **Multiple concurrently-writable regions** are `0.45` W1.
- **The `0.42` `MergePolicy`** resolved split-brain by dropping a loser. **Opt-in
  CRDT convergence** that avoids the drop for safe types is `0.45` W2.
- **Cross-zone async backups (0.41 B5) + locality reads (0.43 W3)** assumed
  ...
  anti-entropy** is `0.45` W3.
- **Operational self-healing + control-plane snapshot/restore (0.43 W6)** recovered
  within a region. **Whole-region failover / DR promotion** is `0.45` W4.
- **Online resharding (0.43 W2)** moved partitions on operator/plan trigger.
  **Capacity signals that drive an autoscaler** to trigger resharding are `0.45` W5.
- **Geo metrics (0.43 W1/W3) + operator surface (0.42 W7)** exposed per-zone data.
  **Per-region staleness and replication-lag SLOs** are `0.45` W6.
...
pub enum WriteAuthority {
    ...
    ActiveActive { home: RegionId },// 0.45: any region writes locally, home reconciles
}
...
per-write across regions naively wastes bandwidth and falls behind. `0.45` needs a
...
the home region for some keys can vanish. `0.45` needs to fail those keys over to a
...
4. Never let an autoscaler request violate a `0.45`/`0.43` invariant — reject loud
...
region-health view. `0.45` adds them so operators can see and alert on the contract
...
- **Full distributed transactions** (serializable cross-node/cross-region multi-key
  commit). Still a hard non-goal; `0.45` adds no transaction semantics.
...
`0.45` reuses the `0.41`–`0.43` shared fault model and harness verbatim
...
`0.45.0` may claim **active-active multi-region grid with bounded staleness** (for
