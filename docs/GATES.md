...
| HC/2 H22 client-plane evidence | `cargo xtask client-plane-ci-check` + `cargo xtask client-plane-spike-check` + pinned Docker `client-plane-docker-interop-check`; scheduled `fuzz_hc2_client_plane`; labelled fixed-host lifecycle soak; `cargo xtask client-plane-ci-admission` | PR Linux/Docker + scheduled/tag/manual fuzz and soak | Four exact, release-required, same-SHA correctness lanes with versioned receipts, intentional missing/red/mixed-SHA admission canaries, pinned toolchains/actions/images, and 30-day retained artifacts. Shared CI and fixed-host soak do not establish absolute performance or production-listener claims. See `docs/operations/HC2_CI_INTEROP_FUZZ_SOAK.md`. |
