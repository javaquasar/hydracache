...
| Redis RESP Docker/client matrix | `HYDRACACHE_RUN_REDIS_COMPAT_CLIENTS=1 cargo test -p hydracache-redis-compat --test redis_clients --locked -- --ignored --nocapture` | CI scheduled/dispatch | mainstream Redis clients and pinned real Redis oracle scenarios for the supported subset, including `MSET`, `SET EX/PX`, `TTL`/`PTTL`, post-expiry reads, auth-required startup, and `rediss://` startup; skips loud unless Docker/client runtimes are available |
