...
| Redis RESP fast surface | `cargo test -p hydracache-redis-compat --locked` + `cargo test -p hydracache-server --test server_lifecycle redis --locked` | CI + verify via workspace tests | RESP2 codec, command translator, unsupported/admin-disabled guardrails, HC extension classification, golden fixtures, fuzz boundary smoke, off-by-default config, distinct listen addresses, and modeled Redis listener drain lifecycle |
