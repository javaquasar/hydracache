...
| Management Center 0.72 claim and evidence contract | `cargo test -p xtask --test management_center_072 --locked` + `cargo xtask management-center-check --release 0.72` + `cargo xtask canary-sweep --release 0.72 --tier fast` + `cargo xtask release-evidence --release 0.72 [--require-ship]` | CI fast + exact-candidate admission | Maps every W0-W14 route/source/claim to executable tests, all 13 NATS-derived storage/cluster/recovery failure classes, reviewed changed-module branches, 88% workspace floor, exact-SHA expected-red canaries and receipts. `--require-ship` rejects missing v0.71 baseline, mixed binaries, Linux resource/soak evidence, coverage receipt, dirty state, stale SHA, skipped tests or any partial taxonomy row. |
