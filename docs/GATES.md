...
| 0.66 operator kind release proof | `cargo xtask evidence-run --release 0.66 --gate env.hydracache-operator-kind-066` | CI scheduled/dispatch prepared-kind lane + release evidence | W5/W11 jointly require the current server/controller image, public committed membership epoch, scale churn without ghost voters, a proven NetworkPolicy-enforcing CNI, and Chaos Mesh `IOChaos` for the slow-disk leg. A missing capability cannot produce the ship receipt. |
...
| 0.66 real-process operational release proof | `cargo xtask evidence-run --release 0.66 --gate env.hydracache-run-066-daemon-process-e2e` | Linux CI scheduled/dispatch + release evidence | Runs W1 snapshot interruption, W2 composed nemesis, W6 mixed binaries, W7 external history, W8 differential process adapter, W10 real suspend/resume, and W12 sender/peer single-flight (`<=1`), cross-term handoff (`<=2` cluster-wide), stale-term cancellation, post-quiescence zero, and `/proc` resource budget. Ship mode requires full Git history and the real `v0.65.0` tag; the pinned development fallback is rejected. |
