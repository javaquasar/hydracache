...
| Redis RESP conformance contract | `cargo xtask doc-check` + `cargo test -p xtask --test doc_check redis_compat --locked` | CI + verify via doc-check / xtask tests | `docs/integrations/redis_compat_conformance.json` is valid, uses pinned `redis-server` oracle images, keeps RESP2 as the 0.63 dialect, gives every supported/candidate command a covering test, and keeps `redis-compat.md` present |
| Redis RESP fast surface | `cargo test -p hydracache-redis-compat --locked` + `cargo test -p hydracache-server --test server_lifecycle redis --locked` | CI + verify via workspace tests | RESP2 codec, command translator, unsupported/admin-disabled guardrails, HC extension classification, golden fixtures, fuzz boundary smoke, off-by-default config, distinct listen addresses, real TCP listener startup, and drain gating |
| Redis RESP Docker/client matrix | `HYDRACACHE_RUN_REDIS_COMPAT_CLIENTS=1 cargo test -p hydracache-redis-compat --test redis_clients --locked -- --ignored --nocapture` | CI scheduled/dispatch | mainstream Redis clients and pinned real Redis oracle scenarios for the supported subset; skips loud unless Docker/client runtimes are available |
