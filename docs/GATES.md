...
| Redis 0.68 keyspace event listener | `cargo test -p hydracache-client-transport-axum --test mutation_events --locked` + `cargo test -p hydracache-redis-compat --test redis_event_listeners --locked` + `cargo test -p xtask --test doc_check redis_compat --locked` | PR Rust/MSRV/Docs via workspace and doc-check | Off-by-default, tenant-fenced metadata bus; Redis `SUBSCRIBE`/`UNSUBSCRIBE`/`PSUBSCRIBE`/`PUNSUBSCRIBE`; RESP2 arrays and RESP3 pushes; binary glob matching; exact/pattern admission bounds by count and retained bytes; auth-before-subscribe; shared RESP/HC2 mutation visibility; explicit lag disconnect and zero active-subscriber accounting. This gate does not claim arbitrary Pub/Sub, `PUBLISH`, replay, values, cross-daemon delivery, or a durable event log. |
