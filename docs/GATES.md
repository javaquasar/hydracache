...
| 0.73 local performance screening foundation | `cargo test -p xtask --test performance_contract_073 --locked`; `cargo xtask performance-contract-check --release 0.73 [--receipt <path>]` | required local/PR structural gate; never release-promotable | Validates exact source/binary/scenario/host/raw-series identity, counterbalanced pair metadata, complete success/rejection/timeout/late/incomplete accounting, frozen 1/8/32/128 concurrency and 25%/60%/85% offered-load lanes. Local receipts must remain `promotable=false` and `numerical_claim_eligible=false`; `--require-ship` fails until dedicated-host and integrated-candidate W0/W10 contracts land. |
