...
| Canary completeness | `cargo test -p xtask --test canary_check --locked` + `cargo xtask canary-check --release 0.64` + `cargo xtask canary-check --release 0.65` + matching fast sweeps | CI fast + scheduled complete sweep | 0.64 keeps the schema-v2 Raft registry; 0.65 adds release-scoped W1-W4 contract canaries. Each normal guard and test-only defect command must produce the registered invariant signature; zero tests, green, timeout, compile error, unrelated panic, skip, or stale receipt is non-evidence |
