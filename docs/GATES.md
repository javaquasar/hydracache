...
| HC/2 H01/H03/H22 client-plane evidence | `cargo xtask client-plane-ci-check` + `cargo xtask client-plane-spike-check` (includes bake-off and two-pass Rust/Java generation) + pinned Docker `client-plane-docker-interop-check`; scheduled `fuzz_hc2_client_plane`; labelled fixed-host lifecycle soak; `cargo xtask client-plane-ci-admission` | PR Linux/Docker + scheduled/tag/manual fuzz and soak | Linux/Docker execute the accepted generation-5 transport manifest, hostile real-mTLS candidate corpus, deterministic code generation, and real production-daemon mTLS/coexistence/startup/drain gates. Four exact, release-required, same-SHA correctness lanes retain versioned receipts with intentional missing/red/mixed-SHA admission canaries, pinned toolchains/actions/images, and 30-day artifacts. Shared CI and fixed-host soak do not establish absolute performance. See `docs/adr/0019-hc2-client-transport.md`, `docs/architecture/HC2_PRODUCTION_LISTENER.md`, and `docs/operations/HC2_CI_INTEROP_FUZZ_SOAK.md`. |
