...
| Management Center 0.72 claim and evidence contract | `cargo test -p xtask --test management_center_072 --locked` + `cargo test -p hydracache-fuzz --test fuzz_corpus_regression --locked` + scheduled `fuzz_management_{envelope,recovery,placement,cursor}` + `cargo xtask management-center-check --release 0.72` + `cargo xtask canary-sweep --release 0.72 --tier fast` + `cargo xtask release-evidence --release 0.72 [--require-ship]` | CI fast corpus + scheduled bounded fuzz + exact-candidate admission | Maps every W0-W14 route/source/claim to executable tests, all 13 NATS-derived storage/cluster/recovery failure classes, reviewed changed-module branches, four bounded management decoder fuzz targets, 88% workspace floor, exact-SHA expected-red canaries and receipts. `--require-ship` rejects missing v0.71 baseline, mixed binaries, Linux resource/soak/fuzz evidence, coverage receipt, dirty state, stale SHA, skipped tests or any partial taxonomy row. |
