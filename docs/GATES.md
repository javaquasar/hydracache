...
| Raft mutation baseline | `cargo test -p xtask --test mutants --locked` + `cargo xtask mutants` | CI fast + scheduled/dispatch | W15 validates `.cargo/mutants.toml`, `docs/testing/mutation-baseline.md`, and any cached `target/hydracache-mutants/report.txt`; `.cargo/mutants.toml` must remain a native cargo-mutants config because the scheduled `Raft Mutation Testing` lane passes it directly to `cargo mutants --config`; fast CI skips loud when no report exists, while the slow lane runs cargo-mutants and fails on every untriaged snapshot/apply/membership survivor |
