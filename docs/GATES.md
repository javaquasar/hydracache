...
| HC/2 H01/H03/H18/H22 client-plane evidence | `cargo xtask client-plane-ci-check` + `cargo xtask client-plane-spike-check` (includes bake-off, two-pass generation, and complete retained compatibility replay) + pinned Docker `client-plane-docker-interop-check`; scheduled `fuzz_hc2_client_plane`; labelled fixed-host lifecycle soak; `cargo xtask client-plane-ci-admission` | PR Linux/Docker + scheduled/tag/manual fuzz and soak | Linux/Docker execute the accepted gRPC transport manifest, current generation-6 contract with bounded generation-5 compatibility/deprecation, hostile real-mTLS corpus, deterministic code generation, retained old/new daemon replay, and real production-daemon mTLS/coexistence/startup/drain gates. `cargo xtask client-plane-compat-check --require-complete` requires all nine cross-version rows. Four exact, release-required, same-SHA correctness lanes retain versioned receipts with intentional missing/red/mixed-SHA admission canaries, pinned toolchains/actions/images, and 30-day artifacts. Shared CI and fixed-host soak do not establish absolute performance. See `docs/adr/0019-hc2-client-transport.md`, `docs/architecture/HC2_COMPATIBILITY_ARTIFACTS.md`, `docs/architecture/HC2_PRODUCTION_LISTENER.md`, and `docs/operations/HC2_CI_INTEROP_FUZZ_SOAK.md`. |
| HC/2 0.68 schema, SDK conformance, packages, and release ledger | `cargo xtask client-schema-check` + `cargo xtask client-conformance --all-sdks` + `cargo xtask client-package-check` + `cargo xtask canary-check --release 0.68` + `cargo xtask release-evidence --release 0.68` | PR Linux + exact-candidate H22 admission | Pins generation 6 across schema and generated metadata, binds nine shared semantics to named Rust/Java/Python tests, consumes the produced crate/JARs/wheel outside the workspace, and maps W0-W12 to dynamic expected-red canaries. Ship mode additionally requires `tool.hc2-release-admission-068`; missing fixed-host/fuzz/hosted receipts, mixed commits, or a green canary fail closed. |
