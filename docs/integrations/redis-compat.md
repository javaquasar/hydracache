...
| `SET NX`, `SET XX`, `SET GET`, `SET KEEPTTL`, `SET EXAT`, `SET PXAT` | `unsupported` | documented divergence | Rejected before dispatch with Redis-shaped `ERR syntax error`. Conditional `SET` is Redis' lock/write-conditional primitive; HydraCache 0.63 does not fake it with a read-then-write race. A future release may support it only with an atomic client-surface conditional write and its own client/oracle matrix. |
